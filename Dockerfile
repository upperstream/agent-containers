...
RUN bash -x hermes_installer.sh --skip-setup --skip-browser --non-interactive ${HERMES_VERSION:+--commit "$(git -C /usr/local/lib/hermes-agent rev-parse --short "${HERMES_VERSION}"^{commit})"} --force-commit
